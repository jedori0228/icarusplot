...
def POT(_axis,_pot,_beam='NuMI'):
    ...
    POTstr = '{:0.2}'.format(_use_pot)+r'$\times 10^{20}$'+' POT'
    if _beam!='':
      POTstr = _beam+', '+POTstr
    return _axis.text( x=_use_x, y=_use_y, s=POTstr,\
...
def Draw2DHist(_axis, _inNPH2D, _xlabel='', _ylabel=''):
    ...
    for i in range(len(_xbins)-1):
        for j in range(len(_ybins)-1):
            _xvals.append(_xbins[i]+_halfDX[i])
            _yvals.append(_ybins[j]+_halfDY[j])
    ...
    for idxX in range(len(_xb)-1):
        for idxY in range(len(_yb)-1):
            ...
            if (_yb[idxY+1]-_yb[idxY])/(_yb[-1]-_yb[0]) > 0.2:
                ytxt = _yb[idxY]+(0.4*(_yb[idxY+1]-_yb[idxY]))
            if _cts[idxX][idxY] < 0.65*np.max(_cts):
                _axis.text( x=xtxt, y=ytxt, s='%1.1f'%(_cts[idxX][idxY]), color='black', fontsize=theFontsize )
            else:
                _axis.text( x=xtxt, y=ytxt, s='%1.1f'%(_cts[idxX][idxY]), color='white', fontsize=theFontsize )
